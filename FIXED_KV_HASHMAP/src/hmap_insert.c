...
{
  int status = 0;
  const hmap_key_t * const ptr_key = (const hmap_key_t * const )in_ptr_key;
  const hmap_val_t * const ptr_val = (const hmap_val_t * const )in_ptr_val;
  register uint32_t hash = set_hash(ptr_key, ptr_hmap);
  register uint32_t probe_loc = set_probe_loc(hash, ptr_hmap);
  val_update_fn_t val_update_fn = ptr_hmap->config.val_update_fn;
  ...
  bkt_t entry; 
  hmap_key_t key = *ptr_key;
  hmap_val_t val = *ptr_val;
  entry.key  = *ptr_key;
  entry.val  = *ptr_val;
  entry.psl  = 0;
  ...
  for ( ; ; ) {
    ...
    hmap_key_t this_key       = bkts[probe_loc].key;
