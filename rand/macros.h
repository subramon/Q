...
// NOTE MARKDOWN TIME RANGE ASSUMPTION
#define make_tmidx(tidx, midx) ( \
    ((uint32_t)tidx) << 4 | \
    ((uint32_t)midx) )
#define make_pidx_from_boh_tmidx(boh, tmidx) ( \
    ((uint32_t)boh) << (4+4) |  \
    ((uint32_t)tmidx) ) 
#define make_pidx(boh, tidx, midx) ( \
    ((uint32_t)boh) << (4+4) | \
    ((uint32_t)tidx) <<4 | \
    ((uint32_t)midx ) )
#endif
