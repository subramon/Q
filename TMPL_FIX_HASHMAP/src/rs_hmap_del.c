    ...
    )
{
  ...
  *ptr_is_found = false;
  if ( in_ptr_val != NULL ) { 
    memset(in_ptr_val, 0, sizeof(${tmpl}_rs_hmap_val_t));
  }
   ...
   */
  ${tmpl}_rs_hmap_bkt_t *this_bkt  = &(bkts[probe_loc]);
  uint32_t prev_probe_loc = probe_loc; 
  for ( ; ; ) {
    // mark this bucket as empty
    memset(&(bkts[probe_loc]), 0, sizeof(${tmpl}_rs_hmap_bkt_t));
    bkt_full[probe_loc] = false; 

    probe_loc++;
    if ( probe_loc == ptr_hmap->size ) { probe_loc = 0; }
    ${tmpl}_rs_hmap_bkt_t *next_bkt = bkts + probe_loc;
    /*
  ...
  if ( ( ptr_hmap->nitems > ptr_hmap->config.min_size ) && 
    ...
    status = ptr_hmap->resize(ptr_hmap, new_size); cBYE(status); 
  }
BYE:
  return status;
}
