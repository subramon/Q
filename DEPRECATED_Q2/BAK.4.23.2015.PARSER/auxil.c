...
{
  int i = 0;
  if ( tbl == NULL ) { WHEREAMI; return false; }
  if ( *tbl == '\0' ) { WHEREAMI; return false; }
  if ( *tbl == '_' ) { WHEREAMI; return false; }
  for ( char *cptr = tbl; *cptr != '\0'; cptr++, i++ ) { 
    if ( i >= MAX_LEN_TBL_NAME ) { WHEREAMI; return false; }
    char letter = *cptr;
...
// STOP FUNC DECL
{
  ...
  if ( ( strcasecmp(X, "Property") == 0 ) ||  /* for debugging */
       ( strcasecmp(X, "Min") == 0 ) || 
       ( strcasecmp(X, "Max") == 0 ) || 
       ( strcasecmp(X, "Sum") == 0 ) || 
       ( strcasecmp(X, "Ndv") == 0 ) || 
       ( strcasecmp(X, "ApproxNdv") == 0 ) || 
       ( strcasecmp(X, "NumNN") == 0 ) || 
       ( strcasecmp(X, "ApproxNdv") == 0 ) ) {
    return true;
  }
  else {
