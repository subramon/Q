    ...
    )
{
  ...
  //----------------------------------------------------
  status = chnk_free_resources( &(g_chnk_hmap.bkts[chnk_where_found].key), 
      &(g_chnk_hmap.bkts[chnk_where_found].val), is_persist);
  cBYE(status);
  ...
  if ( g_vctr_hmap.bkts[vctr_where_found].val.num_chnks == 0 ) { go_BYE(-1); }
  g_vctr_hmap.bkts[vctr_where_found].val.num_chnks--;
