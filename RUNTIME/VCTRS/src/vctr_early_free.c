    ...
    )
{
  ...
  status = vctr_is(vctr_uqid, &vctr_is_found, &vctr_where); cBYE(status);
  ...
  for ( uint32_t chnk_idx = 0; 
    ...
    status = chnk_is(vctr_uqid, chnk_idx, &chnk_is_found, &chnk_where);
    if ( chnk_is_found == false ) { go_BYE(-1); }
    chnk_rs_hmap_val_t *ptr_chnk_val = &(g_chnk_hmap.bkts[chnk_where].val);
