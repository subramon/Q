...
{
  int status = 0;
  REDUCE_sum_sqr_I1_ARGS *ptr_args = (REDUCE_sum_sqr_I1_ARGS *)in_ptr_args;

  if ( idx == 0 ) {
    ptr_args->sum_sqr_val = 0;
    ptr_args->num     = 0;
  }
...
#pragma omp critical (_sum_sqr_I1)
    {
    curr_val = mcr_sum(curr_val, lval);
    }
  } 
  ptr_args->sum_sqr_val += curr_val;
  ptr_args->num     += nR;
  return status;
}
