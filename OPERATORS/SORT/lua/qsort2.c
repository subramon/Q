     ...
     )
{
  ...
  // *2 below to keep space for in1 and in2
  int *pivot_buffer = (char *) alloca (2*size); 
  int   max_thresh   = MAX_THRESH * size;

  if ( total_elems > MAX_THRESH ) {
    int       *lo1 = base_ptr1;
    int       *lo2 = base_ptr2;
    int       *hi1 = lo1 + size * (total_elems - 1);
    int       *hi2 = lo2 + size * (total_elems - 1);
    ...
    while (STACK_NOT_EMPTY) {
      ...
      int *right_ptr;
      {
        int *pivot = pivot_buffer;
        {
          ...
          int *mid1 = lo1 + size * ((hi1 - lo1) / size >> 1);
          int *mid2 = lo2 + size * ((hi2 - lo2) / size >> 1);

          if ( ( *mid1 < *lo1 ) || 
               ( ( *mid1 == *lo1 ) && ( *mid2 < *lo2 ) ) ) {
            SWAP(mid1, lo1, mid2, lo2);
          }
          if ( ( *h1 < *mid1 ) || 
               ( ( *h1 == *mid1 ) && ( *hi2 < *mid2 ) ) ) {
            SWAP(mid1, hi1, mid2, hi2);
          }
          else {
            goto jump_over;
          }
...
jump_over:
          pivot[0] = *mid1; pivot[1] = *mid2;
          // COPY (pivot, mid1, mid2)
          pivot = pivot_buffer;
        }
        left_ptr  = lo + size;
        right_ptr = hi - size; 
