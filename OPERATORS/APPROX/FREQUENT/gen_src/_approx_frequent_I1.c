		...
		)
{
  ...
  temp_cntr_id = (int8_t *)malloc( ((*ptr_active_cntr_siz)+bf_siz)*sizeof(int8_t) );
  ...
  *ptr_active_cntr_siz = kk;
  ...
  memcpy(cntr_id, temp_cntr_id, *ptr_active_cntr_siz*sizeof(int8_t));
    ...
    struct frequent_persistent_data_I1 *data)
{
  ...
  cntr_id = malloc(cntr_siz * sizeof(int8_t)); return_if_malloc_failed(cntr_id);
  cntr_freq = malloc(cntr_siz * sizeof(uint64_t)); return_if_malloc_failed(cntr_freq);
  bf_id = malloc(max_chunk_siz * sizeof(int8_t)); return_if_malloc_failed(bf_id);
  bf_freq = malloc(max_chunk_siz * sizeof(uint64_t)); return_if_malloc_failed(bf_freq);
  packet_space = malloc(max_chunk_siz * sizeof(int8_t)); return_if_malloc_failed(packet_space);
    ...
    struct frequent_persistent_data_I1 *data)
{
  ...
  int8_t **packet_starts = NULL;
  uint64_t *packet_sizs = NULL;
  packet_starts = malloc(NUM_THREADS * sizeof(int8_t*)); return_if_malloc_failed(packet_starts);
  ...
  for (int tid = 0; tid < NUM_THREADS; tid++) {
    ...
    memcpy(packet_starts[tid], src_start, packet_sizs[tid] * sizeof(int8_t));
  }
  ...
  for (int tid = 0; tid < NUM_THREADS; tid++) {
    ...
    qsort_asc_I1(packet_starts[tid], packet_sizs[tid]);
  }
    ...
    uint32_t *out_len)
{
  ...
  *y = NULL;
  *f = NULL;
  *y = malloc(j * sizeof(int8_t)); return_if_malloc_failed(*y);
  *f = malloc(j * sizeof(uint64_t)); return_if_malloc_failed(*f);

  memcpy(*y, data->cntr_id, j * sizeof(int8_t));
...
void free_frequent_persistent_data_I1(struct frequent_persistent_data_I1 *data) {
  if (data == NULL) return;
    ...
    )
{
  ...
  struct frequent_persistent_data_I1 *data = NULL;
  data = malloc(sizeof(struct frequent_persistent_data_I1)); return_if_malloc_failed(data);

  status = allocate_frequent_persistent_data_I1(siz, min_freq, err, 0, data); cBYE(status);

  for (uint64_t i = 0; i < siz; i += data->cntr_siz) {
    int8_t *chunk_start = x + i;
    uint64_t chunk_siz = min(data->cntr_siz, siz - i);
    status = frequent_process_chunk_I1(chunk_start, chunk_siz, data); cBYE(status);
  }

  status = frequent_process_output_I1(data, y, f, out_len); cBYE(status);

 BYE:
  free_frequent_persistent_data_I1(data);
