		...
		)
{
  ...
  temp_cntr_id = (float *)malloc( ((*ptr_active_cntr_siz)+bf_siz)*sizeof(float) );
  ...
  *ptr_active_cntr_siz = kk;
  ...
  memcpy(cntr_id, temp_cntr_id, *ptr_active_cntr_siz*sizeof(float));
    ...
    struct frequent_persistent_data_F4 *data)
{
  ...
  cntr_id = malloc(cntr_siz * sizeof(float)); return_if_malloc_failed(cntr_id);
  cntr_freq = malloc(cntr_siz * sizeof(uint64_t)); return_if_malloc_failed(cntr_freq);
  bf_id = malloc(max_chunk_siz * sizeof(float)); return_if_malloc_failed(bf_id);
  bf_freq = malloc(max_chunk_siz * sizeof(uint64_t)); return_if_malloc_failed(bf_freq);
  packet_space = malloc(max_chunk_siz * sizeof(float)); return_if_malloc_failed(packet_space);
    ...
    struct frequent_persistent_data_F4 *data)
{
  ...
  float **packet_starts = NULL;
  uint64_t *packet_sizs = NULL;
  packet_starts = malloc(NUM_THREADS * sizeof(float*)); return_if_malloc_failed(packet_starts);
  ...
  for (int tid = 0; tid < NUM_THREADS; tid++) {
    ...
    memcpy(packet_starts[tid], src_start, packet_sizs[tid] * sizeof(float));
  }
  ...
  for (int tid = 0; tid < NUM_THREADS; tid++) {
    ...
    qsort_asc_F4(packet_starts[tid], packet_sizs[tid]);
  }
    ...
    uint32_t *out_len)
{
  ...
  *y = NULL;
  *f = NULL;
  *y = malloc(j * sizeof(float)); return_if_malloc_failed(*y);
  *f = malloc(j * sizeof(uint64_t)); return_if_malloc_failed(*f);

  memcpy(*y, data->cntr_id, j * sizeof(float));
...
void free_frequent_persistent_data_F4(struct frequent_persistent_data_F4 *data) {
  if (data == NULL) return;
    ...
    )
{
  ...
  struct frequent_persistent_data_F4 *data = NULL;
  data = malloc(sizeof(struct frequent_persistent_data_F4)); return_if_malloc_failed(data);

  status = allocate_frequent_persistent_data_F4(siz, min_freq, err, 0, data); cBYE(status);

  for (uint64_t i = 0; i < siz; i += data->cntr_siz) {
    float *chunk_start = x + i;
    uint64_t chunk_siz = min(data->cntr_siz, siz - i);
    status = frequent_process_chunk_F4(chunk_start, chunk_siz, data); cBYE(status);
  }

  status = frequent_process_output_F4(data, y, f, out_len); cBYE(status);

 BYE:
  free_frequent_persistent_data_F4(data);
