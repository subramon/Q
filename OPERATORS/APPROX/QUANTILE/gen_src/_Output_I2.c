...
//---------------------------------------------------------------------------
{
  ...
  uint64_t next_quantile_rank = (uint64_t)ceil((jj+1)*(double)siz/num_quantiles); 
  ...
  while ( 1 ) {
    ...
    if ( ( last_packet_incomplete == 1 ) && 
         ( current_loc_lp < last_packet_siz ) ) {
    ...
    while ( current_proc_element_rank >=  next_quantile_rank  && jj < num_quantiles ) {
      ...
      next_quantile_rank = (uint64_t) ceil((jj+1)*(double)siz/num_quantiles);
    }
  }
 BYE:
