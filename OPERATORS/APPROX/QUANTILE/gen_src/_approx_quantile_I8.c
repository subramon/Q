...
//----------------------------------------------------------------------------
{
  ...
  *ptr_estimate_is_good = 1;
  ...
  buffer      = malloc( b * sizeof(int64_t *) ); 
  ...
  for ( unsigned int ii = 0; ii < b; ii++ ) {
    buffer[ii] = (int64_t *) malloc( k * sizeof(int64_t) );
    return_if_malloc_failed(buffer[ii]);
    memset(buffer[ii], 0, k*sizeof(int64_t));
  }
  ...
  lastPacket = (int64_t *)malloc ( k * sizeof(int64_t));
  return_if_malloc_failed(lastPacket);
#ifdef IPP
  ippsZero_32s((int *)lastPacket,k);
#else
  memset(lastPacket, 0, k*sizeof(int64_t));
#endif

  uint64_t lastPacketUsedSiz = 0;
  
  for ( int ii = 0; ii < nT; ii++ ) {
    inputPackets[ii] =  (int64_t *) malloc( k * sizeof(int64_t) );
  }
...
#else
    memset(inputPackets[ii], 0, k*sizeof(int64_t));
#endif
  }
...
#pragma omp parallel for
	...
	memcpy(inputPackets[tid], x+lb, (ub-lb)*sizeof(int64_t));
	inputPacketsUsedSiz[tid] = (ub-lb);
	...
	for ( int ii = 0; ii <= nT; ii++ ) {
	  if ( ( inputPacketsUsedSiz[tid] !=0 ) && 
	    ...
	    last_packet_incomplete = 1; 
	    memcpy(lastPacket, inputPackets[tid],inputPacketsUsedSiz[tid]*sizeof(int64_t));
...
#pragma omp parallel for
    for ( int tid = 0; tid < nT; tid++ ) { 
      ...
      qsort_asc_I8(inputPackets[tid], inputPacketsUsedSiz[tid]);
	...
	status = Collapse_I8(buffer[bufidx1],buffer[bufidx2], weight, bufidx1, bufidx2, b, k);  /* Merge buffer numbers [bufidx1] and [bufidx2] */
	if ( status == -1 ) { 
	  *ptr_estimate_is_good = -1;
	...
	if ( weight[ii] == 0 ) {
      ...
      status = New_I8(inputPackets[tid],buffer[bufidx1],weight,1,bufidx1,b,k);
      if ( status == -1 ) {
	*ptr_estimate_is_good = -1;
	go_BYE(-1);  /* something fundamentally wrong */
      } 
  ...
  if ( last_packet_incomplete == 1 ) {
    qsort_asc_I8(lastPacket, lastPacketUsedSiz);
  }
  ...
  status = Output_I8(buffer,weight,lastPacket, last_packet_incomplete, lastPacketUsedSiz,eff_siz,num_quantiles,y,b,k); 
  if ( status == -1 ) {
    *ptr_estimate_is_good = -1;
    go_BYE(-1); /* something fundamentally wrong */
  }
