...
local function expander_where_ranges(a, b, optargs)
  assert(type(a) == "lVector")
  assert(type(b) == "lVector")
  local sp_fn_name = "Q/OPERATORS/where_ranges/lua/where_specialize"
  ...
  -- we need to know where_ranges we left off
  ...
  local a_name = a:name()
  local b_name = b:name()
  local function where_ranges_gen(chunk_num)
    ...
    repeat
      ...
      if ( a_len ~= b_len ) then 
        print("where_ranges EOV a: " .. a_name .. " = " .. tostring(a:is_eov()))
        print("where_ranges EOV b: " .. b_name .. " = " .. tostring(b:is_eov()))
        print("where_ranges Length a: " .. a_name .. " = " .. a_len)
        print("where_ranges Length b: " .. b_name .. " = " .. b_len)
      end
  ...
  return lVector( { gen = where_ranges_gen, has_nulls = false, qtype = subs.a_qtype } )
end
return expander_where_ranges
